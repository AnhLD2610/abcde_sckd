...
class MultipleNegativesRanking_For_Data_Augment_Loss(nn.Module):
    def __init__(self, scale: float = 20.0, similarity_fct=cos_sim):
        super().__init__()
        self.scale = scale
        self.similarity_fct = similarity_fct
    ...
    def forward(self, list_embedding, list_labels) -> Tensor:
        
        similarities = []
        labels = []
        for i in range (len(list_embedding)):
            for j in range (len(list_embedding)):
                scores = self.similarity_fct(list_embedding[i], list_embedding[j]) * self.scale
                similarities.append(scores)
                label_matrix = (list_labels[i].unsqueeze(1) == list_labels[j].unsqueeze(0)).float()  # Shape: (batch_size, batch_size)
                labels.append(label_matrix)
        # Concatenate all similarities and label matrices
        stacked_similarities = torch.cat([sim.flatten() for sim in similarities], dim=0)  # Shape: (batch_size * batch_size * num_pairs,)
        stacked_labels = torch.cat([label.flatten() for label in labels], dim=0)   # Shape: (batch_size * batch_size * num_pairs,)
        return self.cross_entropy_loss(stacked_similarities, stacked_labels)
        # Create the cross-entropy loss: Positive (1) for matches, Negative (0) for non-matches
        # stacked_labels = stacked_labels.to(device)
        # stacked_similarities = stacked_similarities.to(device)
