...
def augment_mixup(config, tokens, labels):
    """
    Function to concatenate two token sequences and generate new labels where 
    each label list contains two elements: the first label and the second label,
    as well as a matrix for label comparisons.

    Args:
        config: Configuration object containing device and max_length.
        tokens: Tensor of token sequences (batch_size, seq_len).
        labels: Tensor of labels corresponding to each token sequence in the batch.

    Returns:
        augmented_tokens: Tensor containing concatenated token sequences.
        augmented_labels: Tensor where each label is a list of two elements 
                          corresponding to the concatenated tokens.
        label_first: Tensor containing the first labels from each pair.
        label_second: Tensor containing the second labels from each pair.
        new_matrix_labels_tensor: Tensor matrix showing label matches between 
                                  label_first and label_second.
    """
    ...
    for i in random_list_i:
        for j in random_list_j:
            # Remove padding by filtering out zeros from the mask
            token_i = tokens[i][tokens[i] != 0]
            token_j = tokens[j][tokens[j] != 0]

            # Concatenate the two sequences
            combined_tokens = torch.cat((token_i, token_j))

            # Truncate if longer than max_len - 1, and add [102] at the end
            if len(combined_tokens) > max_len - 1:
                combined_tokens = combined_tokens[:max_len - 1]
            combined_tokens = torch.cat((combined_tokens, torch.tensor([102], dtype=combined_tokens.dtype).cuda()))

            # Padding if necessary
            if len(combined_tokens) < max_len:
                padding = torch.zeros(max_len - len(combined_tokens), dtype=combined_tokens.dtype).cuda()
                combined_tokens = torch.cat((combined_tokens, padding))

            # Store the augmented tokens
            augmented_tokens.append(combined_tokens)

            # Store the corresponding labels
            new_label = torch.tensor([labels[i], labels[j]], dtype=torch.long).cuda()
            augmented_labels.append(new_label)
            label_first.append(labels[i])
            label_second.append(labels[j])

    # Convert lists to tensors
    augmented_tokens = torch.stack(augmented_tokens).cuda()
    augmented_labels = torch.stack(augmented_labels).cuda()
    label_first = torch.tensor(label_first).cuda()
    label_second = torch.tensor(label_second).cuda()

    # Create the label comparison matrix
    n = len(label_first)
    m = len(label_second)
    ...
    for i1 in range(n):
        for j in range(m):
            if label_first[i1].item() == label_second[j].item():
                new_matrix_labels[i1][j] = 1.0

    new_matrix_labels_tensor = torch.tensor(new_matrix_labels).cuda()

    # return augmented_tokens, augmented_labels, label_first, label_second, new_matrix_labels_tensor
    return augmented_tokens, new_matrix_labels_tensor




def train_simple_model(config, encoder, dropout_layer, classifier, training_data, epochs, map_relid2tempid):
    data_loader = get_data_loader(config, training_data, shuffle=True, batch_size=config.batch_size)
    ...
    for epoch_i in range(epochs):
        ...
        for step, batch_data in enumerate(data_loader):
            ...
            augment_tokens, augment_labels = augment_mixup(config, tokens, labels)

            reps1, reps2 = encoder(augment_tokens, is_augment = True)
            reps1, _ = dropout_layer(reps1)

            reps2, _ = dropout_layer(reps2)

            loss2 = MultipleNegativesRankingLoss()            
            loss2 = loss2(reps1, reps2, augment_labels) 
